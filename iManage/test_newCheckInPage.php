...
<body>
	...
	<table  style="border:0px solid #ccc; width:100%;">
		<tr>
			...
			<form id="input-form">
				<input placeholder="Check In Date From - Check In Date To" id="rangeDateFrom" type="button" class="button_add radius text" id="orderDate"/>
	...
	function phpSessionExists(key){
		...
		if(key=='_cdate_from'){
			temp= "<?php echo $_SESSION['_cdate_from'] ?>";
			if (temp==='') {
				temp= new Date().toJSON().slice(0,10);
			}
		}
		if(key=='_cdate_to'){
			temp= "<?php echo $_SESSION['_cdate_to'] ?>";
			if (temp==='') {
				temp= new Date().toJSON().slice(0,10);
			}
		}
	...
	// Make it Global
	var customer_name = "", customer_id = "", checkInDatePicker_val = "", temp_si='';
	$(document).ready(function(){
		...
		let checkInDate_arr = checkInDatePicker_val.split("to");
		
		var checkInFrom = phpSessionExists('_cdate_from');
		var checkInTo = phpSessionExists('_cdate_to');
		...
		loadCheckIns(new CheckIn(userId,client,checkInFrom,checkInTo,customer_id,salesperson_id));
		
		const checkInDatePicker = flatpickr("#rangeDateFrom",{
			mode: 'range',
			dateFormat: "Y-m-d",
			disableMobile: "false",
			defaultDate:[checkInFrom,checkInTo],
			onChange: function(selectedDates, dateStr, instance) {
				checkInDatePicker_val = dateStr;
			},
			onReady: function(dObj, dStr, fp, dayElem){
				checkInDatePicker_val = dStr;
			}
		});
		...
		$("#btnSearch").click(function() {
			debugger
			reloadOrderBaseOnChanges(customer_id,salesperson_id,true);
		});
		
		$("#btnClear").click(function() {
			$('#input-form')[0].reset();
			$('#custSelection').val("Choose Customer");
			$("#rangeDateFrom").attr("placeholder", "Check In Date From - Check In Date To");
			$.ajax({
	...
	function reloadOrderBaseOnChanges(customer_id,salesperson_id,btnSearchClicked=false) {
		var userId = '<?php echo $userId; ?>';
		var client = '<?php echo $client; ?>';
		let checkInDate_arr = checkInDatePicker_val.split("to");

		var checkInFrom = checkInDate_arr[0];
		var checkInTo = checkInDate_arr[1];
		debugger
		loadCheckIns(new CheckIn
			(userId,client,checkInFrom,checkInTo,customer_id,salesperson_id)
		,btnSearchClicked);
	}
	
	function loadCheckIns(_, btnSearchClicked = false) {
		console.log(JSON.stringify(_));
		var showCancel = 2;
		var checkInStatus = 1;
		var data = encodeURIComponent('{"searchCheckIns_data" :{ "salespersonId":"' + _.salesperson_id + '","showCancel":"' + showCancel
		+ '","dateFrom":"' + _.checkInFrom + '","dateTo":"' + _.checkInTo + '","check_in_status":"'
		+ checkInStatus + '","customer_status":"' + _.customer_id + '","userId":"' + _.userId
		+ '","client":"' + _.client + '"}}');
		
		jQuery.ajax({
			type: "POST",
			url: "./route.php",
			data: "data=" + data + "&action=getCheckInWithView",
			success: function (msg)
			{
				debugger
				$("#checkInsDiv").empty();
				...
				if(msg) {
					...
					if ((views.length == 0) && btnSearchClicked){
						...
						$("#checkInsDiv").append("<p class='no_orders'>No check in available</p>");
					}else if( views.length == 0 ){
						$("#checkInsDiv").append("<p class='no_orders'>No check in today</p>");
					}else{
						if(views instanceof Array){
							for(let i = 0, len = views.length; i < len; i++){
								let view = views[i];
								$("#checkInsDiv").append(view);
							}
						}
					}
			...
			error: function (xhr, ajaxOptions, thrownError) {
				console.warn(xhr);
				$("#checkInsDiv").append("<p class='no_orders'>Technical error. Please contact support</p>");
			}
		});
	}
