...
<table  style="border:0px solid #ccc; width:100%;">
    ...
    if ($_SESSION['_tilte_name'] ){
        $tilteName=$_SESSION['_tilte_name'];
    }else{
        $tilteName='';
    }
    ...
    <tr style="max-width:100%;overflow: hidden;text-overflow: ellipsis; ">
        <td>
            <select name="titleSelection" id="titleSelection" style="line-height:25px;" class=" radius text_white">
                <option <?php if ($tilteName == '') echo 'selected' ; ?> value="">All Job Type</option>
                <option <?php if ($tilteName == 'CUTTING') echo 'selected' ; ?> value="CUTTING">Cutting</option>
                <option <?php if ($tilteName == 'MACHINING') echo 'selected' ; ?> value="MACHINING">Machining</option>
                <option <?php if ($tilteName == 'SEND TO TRANSPORT') echo 'selected' ; ?> value="SEND TO TRANSPORT">Send To Transport</option>
                <option <?php if ($tilteName == 'READY FOR DELIVERY') echo 'selected' ; ?> value="READY FOR DELIVERY">Ready For Delivery</option>
            </select>
        </td>
    </tr>
    ...
    <tr style="max-width:100%;overflow: hidden;text-overflow: ellipsis; ">
        <td>
            <select name="statusSelection" id="statusSelection" style="line-height:25px;" class=" radius text_white">
                <option <?php if ($statusName == '') echo 'selected' ; ?> value="">All Status</option>
                <option <?php if ($statusName == 'STANDBY') echo 'selected' ; ?> value="STANDBY">StandyBy</option>
                <option <?php if ($statusName == 'IN PROGRESS') echo 'selected' ; ?> value="IN PROGRESS">In Progress</option>
                <option <?php if ($statusName == 'COMPLETED') echo 'selected' ; ?> value="COMPLETED">Completed</option>
                <option <?php if ($statusName == '-CANCELLED-') echo 'selected' ; ?> value="-CANCELLED-">-Cancelled-</option>
            </select>
        </td>
    </tr>
...
<script>
    ...
    $(document).ready(function(){
        ...
        loadOrders(new Order
            (userId,client,orderFrom,orderTo,deliverFrom,deliverTo,customer_id,backlink,salesperson_id,customer_name,customer_code,case_name,status_name,title_name)
        );
        ...
        function loadOrders(_, btnSearchClicked = false) {
            ...
            var data = encodeURIComponent('{"searchOrders_data" :{ "salespersonId":"' + _.userId + '","showCancel":"' + cancel
                ...
                + '","deliveryDateFrom":"' + _.deliverFrom + '","deliveryDateTo":"' + _.deliverTo + '","client":"' + _.client + '","salesperson_id":"' + _.salesperson_id + '","doc":"' + doc_type + '","backlink":"'+_.backlink+'","customer_name":"'+_.customer_name+'","customer_code":"'+_.customer_code+'","case_name":"'+_.case_name+'","status_name":"'+_.status_name+'","title_name":"'+_.title_name+'"}}');
