      ...
      <tbody>
        <tr>
          <td class="top space sub_row">
            ...
            <b class="sub_title" >ONLINE PAYMENT</b><b class="sub_amount" >@online_amount</b><br><br>
            <b class="sub_title">AMBANK</b><b class="sub_amount">@bank_amount</b><br>
          </td>
          @sub_item_row
        </tr>
        <tr>
...
$template['sub_row'] = '
  <b class="sub_title" style="width:80%;text-align:left;">@item_name</b><b class="sub_amount" style="width:20%;">@item_amount</b><br>
';
...
if(
    ...
    for ($k=0; $k <= $diff; $k++) { 
      ...
      $payment_query = "SELECT payment_method, cpd.payment_amount, payment_by, order_id, grand_total FROM cms_order co LEFT JOIN cms_payment cp ON description = order_id AND cp.cancel_status = 0 LEFT JOIN cms_payment_detail cpd on cp.payment_id = cpd.payment_id AND cpd.cancel_status= 0 WHERE delivery_date BETWEEN '".$date." 00:00:00' AND '".$date." 23:59:59' ".$co_sp." AND co.cancel_status = 0 AND co.doc_type = 'cash'";
      $payment = $mysql->Execute($payment_query);

      $payment_array = array();
      $payment_array['tng'] = 0;$payment_array['bank'] = 0;$payment_array['online'] = 0;$payment_array['cash'] = 0;$payment_array['total']=0;
      for ($l=0; $l < count($payment); $l++) { 
        ...
        if($payment[$l]['payment_method'] == 'Wire payment'){
          ...
          }else if($payment[$l]['payment_by'] == '300-001-AM BANK'){
            $payment_array['bank'] += $payment[$l]['grand_total'];
    ...
    foreach($result_set as $order_date=>$data){
      ...
      for ($i=0; $i < count($category_array); $i++) { 
        ...
        $sub_item_html .= '<td class="top space sub_row">';
      ...
      $html_table = str_replace(
        ...
        number_format($payment_array['bank'],2),
