...
if(isset($_GET['custCode']) && isset($_GET['client'])){
    ...
    if(!$connection){
        ...
        $inv_date_condition     = "AND DATE(invoice_date) <= CURRENT_DATE()";
        $dn_date_condition      = "AND DATE(dn_date) <= CURRENT_DATE()";
        $cn_date_condition      = "AND DATE(cn_date) <= CURRENT_DATE()";
        $rcp_date_condition     = "AND DATE(receipt_date) <= CURRENT_DATE()";
        ...
        if($client === 'weiwo'){
            $inv_date_condition     = "AND DATE(invoice_date) >= '2020-07-20' AND DATE(invoice_date) <= CURRENT_DATE()";
            $dn_date_condition      = "AND DATE(dn_date) >= '2020-07-20' AND DATE(dn_date) <= CURRENT_DATE()";
            $cn_date_condition      = "AND DATE(cn_date) >= '2020-07-20' AND DATE(cn_date) <= CURRENT_DATE()";
            $rcp_date_condition     = "AND DATE(receipt_date) >= '2020-07-20' AND DATE(receipt_date) <= CURRENT_DATE()";
            $cf_date_condition      = "AND DATE(cf_date) >= '2020-07-20' AND DATE(cf_date) <= CURRENT_DATE()";
        }
        ...
        if($postDated == '1'){
            ...
            $cn_date_condition      = "AND DATE(cn_date) > CURRENT_DATE()";
            $rcp_date_condition     = "AND DATE(receipt_date) > CURRENT_DATE()";
            $cf_date_condition      = "AND DATE(cf_date) > CURRENT_DATE()";
        ...
        if($isPayment && $is_SmartSQL){
            $inv_date_condition = " AND outstanding_amount <> 0 ";
            $dn_date_condition = " AND outstanding_amount = 0 ";
            $cn_date_condition = " AND cn_knockoff_amount = 0 ";
            $rcp_date_condition = " AND receipt_knockoff_amount = 0 ";
            $cf_date_condition = " AND cf_knockoff_amount = 0 ";
        }
		...
		if($exists){
			...
			$sql                = "SELECT inv.*, {$overdueCond} FROM cms_invoice AS inv WHERE {$cust_query} AND cancelled = 'F' {$inv_date_condition} ORDER BY invoice_date DESC";
		}else{
			$sql                = "SELECT * FROM cms_invoice WHERE {$cust_query} AND cancelled = 'F' {$inv_date_condition} ORDER BY invoice_date DESC";
        }
        ...
        $sql                    = "SELECT * FROM cms_debitnote WHERE {$cust_query}
                                    AND cancelled = 'F' {$dn_date_condition} ORDER BY dn_date DESC";
        ...
        $sql                    = "SELECT * FROM cms_creditnote WHERE {$cust_query} {$cn_date_condition} AND cancelled = 'F' ORDER BY cn_date DESC";
        ...
        $sql                    = "SELECT * FROM cms_receipt WHERE {$cust_query} AND cancelled = 'F' {$rcp_date_condition} ORDER BY receipt_date DESC";
        ...
        $sql                    = "SELECT * FROM cms_customer_refund WHERE {$cust_query} {$cf_date_condition} AND cancelled = 'F' ORDER BY cf_date DESC";
        ...
        if($db->get_num_rows() != 0){
            while ($result = $db->fetch_array()){
                array_push($store,array(
                    TYPE                 => REFUND,
                    "identifier"         => $result['cf_id'],
                    "code"               => $result['cf_code'],
                    "date"               => dateOnly($result['cf_date']),
                    "balance"            => $result['cf_amount'] - $result['cf_knockoff_amount'],
                    "amount"             => $result['cf_amount'],
                    "cancelled"          => $result['cancelled'],
                    "checked"            => isset($result['approved']) ? $result['approved'] : 0,
                    KEY                  => monthAndYear($result['cf_date'])
                ));
                $positive += floatval($result['cf_amount']) - floatval($result['cf_knockoff_amount']);

                $sql_positive += floatval($result['cf_amount']) - floatval($result['cf_knockoff_amount']);
            }
        }
        ...
        if($is_SQLAccounting){
            ...
            foreach ($store as $date=>$data){
                ...
                foreach ($data as $key=>$value){
                    ...
                    if($value[TYPE] === REFUND){
                        $monthlyAmount          -= floatval($value['amount']);
                        $afterKnockOff          = floatval($value['balance']);
                        $refund                 += $afterKnockOff;
                    }
                ...
                $balance                        = ($invoice+$debit) - ($credit+$receipt+$refund);
