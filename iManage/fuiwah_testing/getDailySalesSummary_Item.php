...
if(
    ...
    for ($i=0; $i < count($doc_types); $i++) { 
        ...
        $q = "SELECT cp.category_id,coi.product_name,coi.product_code, sum(quantity) as quantity, coi.unit_price, coi.disc_1, coi.disc_2, coi.disc_3, coi.discount_amount FROM cms_order o LEFT JOIN cms_order_item coi ON o.order_id = coi.order_id AND coi.cancel_status = 0 LEFT JOIN cms_product cp on coi.product_code = cp.product_code WHERE doc_type IN ('{$doc}') AND order_status > 0 AND salesperson_id = '{$salesperson_id}' AND o.cancel_status = 0 AND DATE(order_date) BETWEEN '{$date_from}' AND '{$date_to}' GROUP BY coi.product_code ".$group_by_price." ORDER BY product_code DESC";
            ...
            if(count($data) > 0){
                ...
                foreach ($data as $key => $value) {
                    ...
                    for ($i=0; $i < count($value); $i++) { 
                      ...
                      $item_row .= $i == 0 ? str_replace('@product',$item['product_name'],$html_first_row) : '';
                      $disc = '';
