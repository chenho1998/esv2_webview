...
if(
    ...
    if($isPayment == false){
      ...
      for ($i=0; $i < count($doc_types); $i++) { 
          $type = $doc_types[$i];
          $query = "SELECT order_date,order_id, cust_code, cust_company_name, grand_total,gst_amount,tax, staff_code, driver_name, assistant_name, order_udf FROM cms_order LEFT JOIN cms_login ON login_id = salesperson_id WHERE doc_type = '{$type}' AND order_status > 0 AND salesperson_id = '{$salesperson_id}' AND cancel_status = 0";
