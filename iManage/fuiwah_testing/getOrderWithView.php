...
function getOrderWithView($data){
	...
	$query = "SELECT {$statement_tbl_select} {$select_order_approval} {$packing_select} cms_order.cust_code,cms_order.cust_id,cms_login.name,CONCAT(cms_order.shipping_address1,cms_order.shipping_address2,cms_order.shipping_address3,cms_order.shipping_address4,cms_order.shipping_city,cms_order.shipping_state,cms_order.shipping_zipcode,cms_order.shipping_country) AS shipping, ".$order_from_query."cms_order.order_id,cms_order.grand_total,cms_order.last_print,".$branch_query_select."cms_order.salesperson_id,order_date,delivery_date,grand_total,order_status,cms_order.cancel_status,order_status_last_update_by,order_status_last_update_date,internal_updated_at,cust_remark,cms_customer.cust_company_name,order_delivery_note,staff_code,cms_delivery_info.message, COALESCE(SUM(CASE WHEN (cms_order_item.cancel_status = 0 AND(cms_order_item.packing_status = 1 OR cms_order_item.packing_status = 0))
	...
	SUM( (cms_order_item.packing_status = 2 OR cms_order_item.packing_status = 3) AND cms_order_item.cancel_status = 0 ) AS nostock,cms_order.warehouse_code, cms_order.others_order_status".$order_cutting_date_query." FROM cms_order LEFT JOIN cms_order_item ON (cms_order.order_id = cms_order_item.order_id ) LEFT JOIN cms_customer on cms_customer.cust_code = cms_order.cust_code LEFT JOIN cms_login ON cms_order.salesperson_id = cms_login.login_id ".$branch_query_join." LEFT JOIN cms_delivery_info on cms_order.order_id = cms_delivery_info.order_id {$statement_tbl_cond}
	where ";
	...
	while($result = $db->fetch_array())
	{
		...
		if($result['order_status']=='0'){
			...
			if(!empty($select_order_approval) && !$isCustomer && $result["order_from"] == 'C'){
				if($result['order_approved'] == 2){
					$a_view .='<p class="title" style="font-weight:bold;color:rgb(255,85,45);width:33%;margin:0px;"> Rejected </p>';
				}
				if($result["order_comment"]){
					$a_view .= '<p class="message non_important_text" style="color:rgb(255,85,45);margin:0px;">Reason: '.$result["order_comment"].' </p>';
				}
			}
			...
			if(in_array($data['client'],$delivery_module)){
				...
				if(!empty($select_order_approval) && !$isCustomer && $result["order_from"] == 'C'){
					...
					}else if($result['order_approved'] == 1){
						$c_view .='<p class="title" style="font-weight:bold;color:#3CB371;width:33%;margin:0px;"> Approved </p>';
					}
				}else{
					if($result["message"] && $isMsgValid){
