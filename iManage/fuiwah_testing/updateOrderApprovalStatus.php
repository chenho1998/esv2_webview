...
{  
    ...
    if($client == 'oasisqi' || $client == 'fuiwah'){
        if($action == 'approve'){
            $db->query("update cms_order set order_approved = 1 where order_id = '{$order_id}'");
        }
        if($action == 'reject'){
            $db->query("update cms_order set order_status = 0, order_comment = '{$comment}', order_approved = 2 where order_id = '{$order_id}'");
        }
        $json_data['result'] = 1;
