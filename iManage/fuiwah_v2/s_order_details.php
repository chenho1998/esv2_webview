...
<script>
    ...
    function openModal(ipad_item_id, product_code, unit_uom, quantity, unit_price, disc_1, disc_2, disc_3, salesperson_remark, discount_method, product_name, exchanged_qty, rejected_qty, less_qty, new_qty, editted_qty){
        debugger;
        var modal = document.getElementById("myModal");
        let client = "<?php echo $client; ?>";
        ...
        $.ajax({
            ...
            success: function(msg)
            {
                ...
                if(data){
                    $(".content").empty();
                    var view ="<p style='font-weight:bold;'>"+product_name+"</p><input class='numberic' style='width:32.6%;line-height:35px;margin-right:1%;display:none;' type='text' id='price' value="+unit_price+" placeholder='Price' onClick='this.select();'><input class='numberic' style='width:49.5%;line-height:35px;margin-right:1%;' type='text' id='quantity' value="+quantity+" placeholder='Quantity' onClick='this.select();' disabled>";

                    var select = "<select style='width:49.5%;height:35px;border-radius:3px;border:1px solid whitesmoke;' id='unit_uom'>";
                    ...
                    var exchanged = 'EXCHANGED';
                    var rejected = 'REJECTED';
                    var less = 'LESS QTY';

                    if(editted_qty != 0){
                        var ori_qty = editted_qty;
                    }else{
                        var ori_qty = quantity;
                    }

                    var net_total = parseInt(ori_qty) +  parseInt(new_qty) -  parseInt(exchanged_qty) -  parseInt(rejected_qty) -  parseInt(less_qty);

                    view += "<br>"+discount;

                    view+= "<table style='margin-top:1%;width:100%;padding: 0px;border-radius: 0px;' border='collapse'><tr><td style='width:50%;border:1px solid #000;padding:5px;background-color:#000; color:#fff; border-radius:0px;'>ORIGINAL</td><td style='width:50%;border:1px solid #000;'><input class='numberic' style='border: 0px solid white; width: 100%; padding: 5px;background-color:#000; color:#fff;border-radius:0px;' type='text' id='original_qty' value='"+ori_qty+"' disabled></td></tr><tr><td style='width:50%;border:1px solid #bfa084;padding:5px;'>"+additional+"</td><td style='width:50%;border:1px solid #bfa084;'><input class='numberic' style='border: 0px solid white; width: 100%; padding: 5px;' type='text' id='new_qty' value='"+new_qty+"' onkeyup='updateNetTotal()' onClick='this.select();' placeholder='ADD. QTY'></td></tr><tr><td style='width:50%;border:1px solid #bfa084;padding:5px;'>"+exchanged+"</td><td style='width:50%;border:1px solid #bfa084;'><input class='numberic' style='border: 0px solid white; width: 100%; padding: 5px;' type='text' id='exchanged_qty' value='"+exchanged_qty+"' onkeyup='updateNetTotal()' onClick='this.select();' placeholder='EXCH. QTY'></td></tr><tr><td style='width:50%;border:1px solid #bfa084;padding:5px;'>"+rejected+"</td><td style='width:50%;border:1px solid #bfa084;'><input class='numberic' style='border: 0px solid white; width: 100%; padding: 5px;' type='text' id='rejected_qty' value='"+rejected_qty+"' onkeyup='updateNetTotal()' onClick='this.select();' placeholder='REJ. QTY'></td></tr><tr><td style='width:50%;border:1px solid #bfa084;padding:5px;'>"+less+"</td><td style='width:50%;border:1px solid #bfa084;'><input class='numberic' style='border: 0px solid white; width: 100%; padding: 5px;' type='text' id='less_qty' value='"+less_qty+"' onkeyup='updateNetTotal()' onClick='this.select();' placeholder='LESS QTY'></td></tr><tr><td style='width:50%;border:1px solid #000;padding:5px;background-color:#000; color:#fff; border-radius:0px;'>NET TOTAL</td><td style='width:50%;border:1px solid #000;border-radius:0px;'><input class='numberic' style='border: 0px solid white; width: 100%; padding: 5px;background-color:#000; color:#fff;border-radius:0px;' type='text' id='net_total' value='"+net_total+"' disabled></td></tr></table>";

                    view+= "<input type='text' id='salesperson_remark' value='"+salesperson_remark+"' placeholder='Remark' style='width:100%;line-height:35px;margin-top:1%;'>"

                    view+= "<input type='text' class='numberic' id='discount_method' value='"+discount_method+"' style='width:100%;display: none;'>";

                    view+= "<div style='width:100%;margin-top:5px;line-height:0px;'><button class='btn-delete' onclick='updateOrderItem("+ipad_item_id+",1)' style='width:49.5%;float:left;margin-right:1%;'>DELETE</button><button class='btn-update' style='width:49.5%;float:left;' onclick='updateOrderItem("+ipad_item_id+",0)' >UPDATE</button></div>";

                    //view += "<br>"+discount+"<input type='text' id='salesperson_remark' value='"+salesperson_remark+"' placeholder='Remark' style='width:100%;line-height:35px;margin-top:1%;'><input type='text' class='numberic' id='discount_method' value='"+discount_method+"' style='width:100%;visibility: hidden;'><div style='width:100%;margin-top:5px;line-height:0px;'><button class='btn-delete' onclick='updateOrderItem("+ipad_item_id+",1)' style='width:49.5%;float:left;margin-right:1%;'>DELETE</button><button class='btn-update' style='width:49.5%;float:left;' onclick='updateOrderItem("+ipad_item_id+",0)' >UPDATE</button></div>";
    ...
    function updateNetTotal(){
        ...
        if($('#exchanged_qty').val() != ''){
            var exchanged_qty = $('#exchanged_qty').val();
        }else{
            var exchanged_qty = 0;
        }
        ...
        if($('#less_qty').val() != ''){
            var less_qty = $('#less_qty').val();
        }else{
            var less_qty = 0;
        }
        
        var net_total = parseInt(original_qty) + parseInt(new_qty) - parseInt(exchanged_qty) - parseInt(rejected_qty) - parseInt(less_qty);

        if(net_total != ''){
            document.getElementById('net_total').value = net_total.toString();
        }
    }
    ...
    function updateOrderItem(ipad_item_id,status){
        ...
        var quantity = document.getElementById('original_qty').value;
        ...
        var exchanged_qty = 0;
        if(document.getElementById('exchanged_qty')){
            if(document.getElementById('exchanged_qty').value != ''){
                var exchanged_qty = document.getElementById('exchanged_qty').value;
            }
        }

        var rejected_qty = 0;
        if(document.getElementById('rejected_qty')){
            if(document.getElementById('rejected_qty').value != ''){
                var rejected_qty = document.getElementById('rejected_qty').value;
            }
        }

        var less_qty = 0;
        if(document.getElementById('less_qty')){
            if(document.getElementById('less_qty').value != ''){
                var less_qty = document.getElementById('less_qty').value;
            }
        }

        var new_qty = 0;
        if(document.getElementById('new_qty')){
            if(document.getElementById('new_qty').value != ''){
    ...
    function updateGrn(order_id){

        let client = "<?php echo $client; ?>";
        let grn_no = document.getElementById('grn_no').value;

        var data = encodeURIComponent('{"updateGrn_data" :{ "order_id":"' + order_id + '","client":"' + client + '","grn_no":"'+ grn_no +'"}}');
        debugger;
        $.ajax({
            type: "POST",
            url: "./../route.php",
            data: "data=" + data + "&action=updateGrn",
            success: function(msg)
            {
                debugger;
                var decodedJson = JSON.parse(msg);
