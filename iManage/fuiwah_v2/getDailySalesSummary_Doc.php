...
<table style="width:100%;border:0px solid white;">
    ...
    <col width="33%">
    <tr>
        <th align="left">@customer_name <br><i style="font-size:10px;">@cust_company_name</i>@salesperson</th>
        <th align="left">@doc_id <br>@doc_date <br>@driver</th>
        <th align="left"><br>@delivery_date <br>@assistant</th>
    </tr>
    @item_row
</table>';
...
if(
    ...
    if($isPayment == false){
      ...
      for ($i=0; $i < count($doc_types); $i++) { 
          $type = $doc_types[$i];
          $query = "SELECT order_date,order_id, cms_order.cust_code, cms_order.cust_company_name, cust_remark, (SELECT SUM(sub_total) FROM cms_order_item WHERE cms_order_item.order_id = cms_order.order_id AND cancel_status = 0) AS grand_total,gst_amount,tax, staff_code, driver_name, assistant_name, order_udf, delivery_date FROM cms_order LEFT JOIN cms_customer ON cms_customer.cust_code = cms_order.cust_code LEFT JOIN cms_login ON login_id = salesperson_id WHERE doc_type = '{$type}' AND order_status > 0 AND salesperson_id = '{$salesperson_id}' AND cancel_status = 0";
          
          $query .= " AND DATE(delivery_date) BETWEEN '{$date_from}' AND '{$date_to}';";
          ...
          for ($j=0; $j < count($documents); $j++) { 
              ...
              $documents[$j]['cust_remark'] = $documents[$j]['cust_remark'] != $documents[$j]['cust_company_name'] ? $documents[$j]['cust_remark'] : '';
        ...
        foreach($result_set as $doc_type=>$data){
          ...
          for ($i=0; $i < count($data); $i++) { 
            ...
            $doc_details[] = array(
              ...
              'delivery_date'=>malaysianDate($data[$i]['delivery_date']),
        ...
        foreach($result_set as $doc_type=>$data){
          ...
          if($numofRecord > 0){
              ...
              for ($i=0; $i < $numofRecord; $i++) { 
                  ...
                  $salesman_name = '';
                  if($doc['order_udf']){
                    $udf = json_decode($doc['order_udf'],true);
                    for ($p=0; $p < count($udf); $p++) {
                      $each = $udf[$p]; 
                      if($each['code'] == 'ref1'){
                        $salesman_name = $each['value'];
                      }
                    }
                  }
                  ...
                  $innerHTML .= str_replace(
                      ...
                      array(
                          $doc['cust_remark'] ? '['.$doc['cust_code'].']  '.$doc['cust_remark'] : '['.$doc['cust_code'].']  '.$doc['cust_company_name'],
                          $doc['payment_method'] ? $doc['order_id'] . ' | '.$doc['payment_method'] : $doc['order_id'],
                          malaysianDate($doc['order_date']),
                          malaysianDate($doc['delivery_date']),
                          ...
                          '</span>'
                          : '',
                          $doc['cust_remark'] ? $doc['cust_company_name'] : ''
                      ),
                      $html_cust_row
                  );
              }
