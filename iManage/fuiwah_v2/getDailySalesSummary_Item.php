...
if(
    ...
    $q = "SELECT cp.category_id,coi.product_name,coi.product_code, sum(quantity) as quantity, coi.unit_price, coi.disc_1, coi.disc_2, coi.disc_3, coi.discount_amount FROM cms_order o LEFT JOIN cms_order_item coi ON o.order_id = coi.order_id AND coi.cancel_status = 0 LEFT JOIN cms_product cp on coi.product_code = cp.product_code WHERE order_status > 0 AND salesperson_id = '{$salesperson_id}' AND o.cancel_status = 0 AND coi.cancel_status = 0 AND DATE(delivery_date) BETWEEN '{$date_from}' AND '{$date_to}' GROUP BY coi.product_code, coi.unit_price ORDER BY product_code DESC";
    ...
    for ($i=0; $i < count($doc_types); $i++) { 
        ...
        $q = "SELECT cp.category_id,coi.product_name,coi.product_code, sum(quantity) as quantity, coi.unit_price, coi.disc_1, coi.disc_2, coi.disc_3, coi.discount_amount FROM cms_order o LEFT JOIN cms_order_item coi ON o.order_id = coi.order_id AND coi.cancel_status = 0 LEFT JOIN cms_product cp on coi.product_code = cp.product_code WHERE doc_type IN ('{$doc}') AND order_status > 0 AND salesperson_id = '{$salesperson_id}' AND o.cancel_status = 0 AND coi.cancel_status = 0 AND DATE(delivery_date) BETWEEN '{$date_from}' AND '{$date_to}' GROUP BY coi.product_code ".$group_by_price." ORDER BY product_code DESC";
    ...
    if($printType == 'bluetooth'){
        echo json_encode(array('data'=>$stock_balance ? $stock_balance_arr : $bt_array));
        return;
    }else{
