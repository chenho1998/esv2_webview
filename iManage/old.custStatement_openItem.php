...
<?php
    ...
    if($dont_show_ko){
        $invoice_out_q = "SELECT i.*,date(k.doc_date) as doc_date, if(k.doc_code is null,'','*') as aestric,k.doc_code,k.doc_type,DATE_FORMAT(invoice_date,'%d/%m/%Y') AS formatted_date FROM cms_invoice i
    ...
    invoice_code,cust_code,invoice_date,invoice_due_date,invoice_amount,i.salesperson_id,inv_udf,cancelled,
                   DATE_FORMAT(invoice_date,'%d/%m/%Y') AS formatted_date FROM cms_invoice i
    ...
    while($result = $db->fetch_array()){
        ...
        if(!empty($result['doc_date']) && !$same_date){
            ...
            $show_outstanding = true;//(strtotime($result['doc_date']) > strtotime($_date_to))?!$inRange:$inRange;
        }
    ...
    $transaction = unqByObjProp($transaction,'id');
    ...
    $view[] =  '<tr>
            ...
            <td></td>
            <td style="text-align:center;font-size:10px;font-weight:bold">Description</td>
            <td></td>
    ...
    $view[] = '<div style="margin-top:10px;font-size:7px">Statement Date From: '.$_date_from.' Date To: '.$_date_to.' Generated At: '.date('d/m/Y h:i:s').'</div>';
