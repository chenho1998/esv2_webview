...
<table  style="border:0px solid #ccc; width:100%;">
    <tr>
        ...
        <form id="input-form">
            <input placeholder="Date From - Date To" id="rangeDateFrom" type="button" class="button_add radius text" id="orderDate"/>
        </form>
        </td>
    </tr>
    <tr>
...
$(document).ready(function(){
    ...
    var orderFrom           = phpSessionExists('_cdate_from');
    var orderTo             = phpSessionExists('_cdate_to');
...
function loadItems(_) {
    var data = encodeURIComponent('{"searchRollingCash_data" :{ "salespersonId":"' + _.user_id + '","client":"' + _.client + '","dateFrom":"' + _.date_from + '","dateTo":"' + _.date_to + '"}}');
    jQuery.ajax({
        type: "POST",
        url: "./route.php",
        data: "data=" + data + "&action=getCashRollingView",
        success: function (msg) {
           
            $('#ordersDiv').empty();
            if(msg){
                $("#ordersDiv").append(msg);
            }

            if(msg.length==0){
                
                $("#ordersDiv").append("<p class='no_orders'>No Records Available</p>");
            }
        },
...
function clearSession(){
    $("#rangeDateFrom").attr("placeholder", "Date From - Date To");

    $.ajax({
        url: "clearSession.php?action=clearSessionRC",
        type: "POST",
        contentType: false,
        cache: false,
        processData:false,
...
.bottom-bar{
    ...
    transition: 0.3s;
    z-index: 1;
    width:90%;margin:auto;padding:5px;background-color:#147efb;height:50px;border-radius:5px;
    text-align: center;
    color:white;
    font-size:12px;
    line-height: 40px;
