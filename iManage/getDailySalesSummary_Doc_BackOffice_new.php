...
if(
    ...
    // if($salesperson_id == ''){
    //   $login_id = $mysql->Execute("SELECT login_id FROM cms_login WHERE login_status = 1");
    //   for($l=0;$l<count($login_id);$l++){
    //     array_push($arr,$login_id[$l]['login_id']);
    //   }
    // }
    ...
    for($s=0;$s<count($arr);$s++){
      ...
      if($isPayment == false){
        for ($i=0; $i < count($doc_types); $i++) {
            $type = $doc_types[$i];
            $query = "SELECT order_date,order_id, cms_order.cust_code, cms_order.cust_company_name, cust_remark, grand_total,gst_amount,tax, staff_code, driver_name, assistant_name, order_udf, delivery_date FROM cms_order LEFT JOIN cms_customer ON cms_customer.cust_code = cms_order.cust_code LEFT JOIN cms_login ON login_id = salesperson_id 
                      WHERE doc_type = '{$type}' AND order_status > 0 AND salesperson_id  = '{$arr[$s]}' AND cancel_status = 0";
