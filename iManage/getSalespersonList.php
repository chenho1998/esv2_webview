...
function getSalespersonList($data)
{
    ...
    if (strpos($salespersonid,',') !== false){
        $query="SELECT login_id,name FROM cms_login WHERE login_id IN ($salespersonid) AND login_status = 1 AND role_id in (2,9,15,10)";
    }else if($salespersonid!=0 && $role_id != 10){
        $query="SELECT login_id,name FROM cms_login WHERE login_id IN ($salespersonid) AND login_status = 1 AND role_id in (2,9,15,10)";
    }else if($role_id == 10){
        $query = "SELECT login_id,name FROM cms_login WHERE (role_id=2 OR role_id=10) AND login_status = 1";
    }else{
        $query="SELECT login_id,name FROM cms_login WHERE role_id in (2,9,15) AND login_status = 1"; //role_id 2 means salesperson, not admin or anything
    }
