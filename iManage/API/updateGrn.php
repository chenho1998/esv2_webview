		...
		while($result = $db->fetch_array())
		{
			...
			if(count($order_udf) > 0){
				...
				for ($i=0; $i < count($order_udf); $i++) { 
					if($order_udf[$i]['code'] == 'grn'){
						$order_udf[$i]['value'] = $data['grn_no'];
						$found = 1;
					}
				}
				...
				$update = '[{"code": "grn", "title": "GRN No", "item": { "name": "GRN No", "required": "0", "value": "1" }, "selected": 0, "type": "input", "value": "'.$data['grn_no'].'"}]';
