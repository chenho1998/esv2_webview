...
<script>
    ...
    var filter_cust,filter_salesperson,custSelection,offset = 0,acknowledge;
    ...
    function showModal(json){
        let param = json.details;
        modal.style.display = "block";
        var view = '';
        for (let i = 0; i < param.length; i++) {
            const element = param[i];
            const checked = element.acknowledged == 1 ? 'checked':'';
            view += `<div class="box">
                    <input onChange="acknowledge(${element.id},${element.acknowledged})" type="checkbox" id="${element.id}" name="${element.id}" style="margin-top:5px;margin-right:5px;float:right"${element.acknowledged?'checked':''}>
                    <label id="${element.id}" style="font-size:12px;color:grey;margin-left:5px">ITEM RECEIVED</label><br>
                        
                        <p style="color:#147efb;font-weight:bold">${element.product_code}</p>
                        <p style="color:#000;font-weight:bold">${element.product_name}</p>
                        <p style="color:#000;font-weight:normal;font-style:italic;">${element.quantity} QTY</p>
                        <div class="row">
                            <div class="column-box">
                                <center style="color:black;font-weight:bold;"> ${element.from_location}</center>
                            </div>
                            <div class="column-box">
                                <center style="color:#147efb;font-weight:bold;">${element.to_location}</center>
                            </div>
                        </div>
                    </div>`;
        }
    ...
    $(document).ready(function(){
        ...
        acknowledge = function acknowledge(id, current){
            current ^= 1;
            var data = encodeURIComponent('{"updateStkTrItemAck_data" :{ "id":"' + id + '","acknowledged":"' + current + '","client":"<?php echo $client; ?>"}}');

            jQuery.ajax({
                type: "POST",
                url: "./route.php",
                data: "data=" + data + "&action=updateStkTrItemAck",
        ...
        function loadOrders(_, btnSearchClicked = false) {
            ...
            var data = encodeURIComponent('{"searchStockTransfer_data" :{ "salespersonId":"' + _.userId + '","showCancel":"' + cancel
            ...
            jQuery.ajax({
                type: "POST",
                url: "./route.php",
                data: "data=" + data + "&action=getStockTransferWithView",
                success: function (msg) {
