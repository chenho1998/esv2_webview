...
{  
    ...
    $result = mysql_query("SHOW COLUMNS FROM `cms_customer_salesperson` LIKE 'active_status'");
    $exists = (mysql_num_rows($result))?TRUE:FALSE;

    if(!$exists){
        $db->query("ALTER TABLE cms_customer_salesperson ADD COLUMN active_status INT DEFAULT 1;");
    }

    if($salespersonid!='0'){
        $query = "SELECT cms_customer.cust_id, cms_customer.cust_company_name, cms_customer.cust_code FROM cms_customer_salesperson LEFT JOIN cms_customer ON cms_customer_salesperson.customer_id = cms_customer.cust_id WHERE cms_customer.customer_status = 1 AND cms_customer_salesperson.salesperson_id IN (".$salespersonid.") AND active_status = 1 ORDER BY cms_customer.cust_company_name";
    }else{
        $query = "SELECT cms_customer.cust_id, cms_customer.cust_company_name, cms_customer.cust_code FROM cms_customer_salesperson LEFT JOIN cms_customer ON cms_customer_salesperson.customer_id = cms_customer.cust_id WHERE cms_customer.customer_status = 1 AND active_status = 1 ORDER BY cms_customer.cust_company_name";
    }
