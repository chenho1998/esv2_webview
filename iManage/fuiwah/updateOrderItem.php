...
function updateOrderItem($data)
{
    ...
    $net_total = $data['quantity'] - $data['exchanged_qty'] - $data['rejected_qty'] - $data['less_qty'];

    $query = "UPDATE cms_order_item SET editted_quantity= CASE WHEN editted_quantity = 0 THEN quantity ELSE editted_quantity END, unit_price = ".$data['price'].", unit_uom = '".$data['uom']."', quantity = ".$net_total.", disc_1 = '".$data['disc_1']."', disc_2 = '".$data['disc_2']."', disc_3 = '".$data['disc_3']."',salesperson_remark = '".$data['remark']."',cancel_status = ".$data['status'].", sub_total = '".$sub_total."', discount_amount = '".$discount."', discount_method ='".$discount_method."', exchanged_qty = exchanged_qty + ".$data['exchanged_qty'].", rejected_qty = rejected_qty + ".$data['rejected_qty'].", less_qty = less_qty + ".$data['less_qty']." WHERE order_item_id = ".$data['ipad_item_id'] ." AND order_id = '{$order_id}' ";
