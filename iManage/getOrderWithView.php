...
function getOrderWithView($data){
	...
	if($client == 'ventureflex'){
		file_put_contents('query.log',json_encode($data));
	}
	...
	$hide_pricing = $config['Hide_Price'];
	$hide_price = in_array($client,$hide_pricing['hide_pricing']);

	$sql_accounting = $config['SQL'];
	$sql_client = in_array($client,$sql_accounting['sql_client']);

	$edit_warehouse = $config['Edit_Warehouse'];
	$allow_edit_warehouse = in_array($client,$edit_warehouse['edit_warehouse']);

	$edit_order_date = $config['Edit_Order_Date'];
	$allow_edit_order_date = in_array($client,$edit_order_date['edit_order_date']);

	$approve_from_sp = $config['Approve_From_SP'];
	$approve_from_sp_setting = in_array($client,$approve_from_sp['approve_from_sp']);

	$show_udf = $config['Show_UDF'];
	$show_udf_po = in_array($client,$show_udf['po_no']);
	...
	$approve_comment_setting = 0;
	$db->query("SELECT status FROM cms_mobile_module WHERE module = 'app_selforder_status'");
	while($result = $db->fetch_array()){
		$approve_comment_setting = 1;
	}
	...
	$tax_select = $tax_module ? "cms_order.tax," : "";
	...
	if($client == 'bigbathxil' && $data['doc'] == 'sales'){
		$doc_type = " AND doc_type IN ('selforder','sales') ";
	}
	...
	if($statement_tbl_exists){
		...
		$statement_tbl_select = " sg.id, sg.is_signed, sg.statement_path, ";
	}
	...
	$query = "SELECT {$tax_select}{$statement_tbl_select} {$select_order_approval} {$packing_select} cms_order.cust_code,cms_order.cust_id,cms_login.name,CONCAT(cms_order.shipping_address1,cms_order.shipping_address2,cms_order.shipping_address3,cms_order.shipping_address4,cms_order.shipping_city,cms_order.shipping_state,cms_order.shipping_zipcode,cms_order.shipping_country) AS shipping, ".$order_from_query."cms_order.order_id,cms_order.grand_total,cms_order.last_print,".$branch_query_select."cms_order.salesperson_id,order_date,delivery_date,grand_total,order_status,cms_order.cancel_status,order_status_last_update_by,order_status_last_update_date,internal_updated_at,cust_company_name,order_udf,order_delivery_note,staff_code,cms_delivery_info.message, COALESCE(SUM(CASE WHEN (cms_order_item.cancel_status = 0 AND(cms_order_item.packing_status = 1 OR cms_order_item.packing_status = 0))
	...
	SUM( (cms_order_item.packing_status = 2 OR cms_order_item.packing_status = 3) AND cms_order_item.cancel_status = 0 ) AS nostock,cms_order.warehouse_code, cms_order.others_order_status".$order_cutting_date_query." FROM cms_order LEFT JOIN cms_order_item ON (cms_order.order_id = cms_order_item.order_id ) LEFT JOIN cms_login ON cms_order.salesperson_id = cms_login.login_id ".$branch_query_join." LEFT JOIN cms_delivery_info on cms_order.order_id = cms_delivery_info.order_id {$statement_tbl_cond}
	where ";
	...
	if($allow_constructor){
		...
		(SELECT cutting_title FROM cms_acc_order_cutting_date WHERE order_id = cms_acc_existing_order.order_id ORDER BY updated_at DESC LIMIT 1) AS cutting_title FROM cms_acc_existing_order WHERE ";
	...
	if($data['showCancel'] == 0){
		$query .= " AND ".$order_table.".cancel_status = 0";
	}
	...
	$query .= in_array($data['client'],$order_by_column['order_id']) ? " ".$order_table.".order_id" : " ".$order_table.".order_date";
	...
	if($payment_display){
		// very bad idea. please modify later
		$db3 = clone $db;
		$db3->query($query);

		$order_ids = array();
		while($result = $db3->fetch_array()){
			$order_ids[] = $result["order_id"];
		}
		$order_ids = "'" . implode("','",$order_ids) . "'";
		$payment_array = array();

		$payment_query = "SELECT * FROM cms_payment WHERE doc_id IN ({$order_ids})";
		

		if($data['showCancel'] == 0){
			$payment_query .= " AND cancel_status = 0";
		}else{
			$payment_query .= " AND cancel_status > 0";
		}
		$db3->query($payment_query);
		if($db3->get_num_rows() > 0)
		{
			while($result = $db3->fetch_array())
			{
				$payment_array[$result['doc_id']] = $result['payment_amount'];
			}
		}
	}
	...
	$t_view='<p style="text-align:center; line-height: 25px;" class="dropdown-div text_white"> Transferred orders (@t_orders_count)</p>';
	$c_view='<p style="text-align:center; line-height: 25px;" class="dropdown-div text_white"> Confirmed orders (@c_orders_count)</p>';
	$a_view='<p style="text-align:center; line-height: 25px;" class="dropdown-div text_white"> Active orders (@a_orders_count)</p>';
	$p_view= '<p style="text-align:center; line-height: 25px;" class="dropdown-div text_white"> Posted orders (@p_orders_count)</p>';
	...
	while($result = $db->fetch_array())
	{
		...
		$label_customer_name = $result['cust_company_name'];
		if($client == 'fuiwah'){
		...
		if (!empty($result['order_udf']) && $order_udf && is_array($order_udf) && count($order_udf) > 0){
			for($u = 0; $u < count($order_udf); $u++){
				$order_udf_obj = $order_udf[$u];
				if (!empty($order_udf_obj['rest']) && isset($order_udf_obj['rest']['realm']) && !empty($order_udf_obj['value']) && $order_udf_obj['rest']['realm'] == "cust_company_name"){
					$label_customer_name = $order_udf_obj['value'];
				}

				if($show_udf_po){
					if($order_udf_obj['code'] == 'po_no'){
						if($order_udf_obj['value']){
							$show_po_view = '<p class="non_important_text" style="font-weight:bold;">PO Number: '.$order_udf_obj['value'].' </p>';
						}
					}
				}
			}
		...
		if($statement_tbl_exists){
			if($result['statement_path']){
				$statement_pdf_btn = 
				'<button id="pdf_button" class="img_button" style="margin-left:5px;font-width: 10vw;" onclick="window.ReactNativeWebView.postMessage(`statement_id:'.$result['statement_path'].'`)">Reconcl.</button>';
			}else{
				$statement_pdf_btn = '';
		...
		if($tax_module){
			$real_total += $result['tax'];
		}

		$payment_view = '';
		if($payment_display){
			if($payment_array[$result['order_id']]){
				$payment_amount = number_format($payment_array[$result['order_id']],2);
				if($payment_amount != number_format($real_total,2)){
					$payment_view = '<p class="dates" style="font-weight:bold;color:rgb(255,85,45);">Partial Paid RM'.$payment_amount.'</p>';
				}else{
					$payment_view = '<p class="dates" style="font-weight:bold;color:#3CB371;">Fully Paid: RM'.$payment_amount.'</p>';
				}
			}else{
				$payment_view = '<p class="dates" style="font-weight:bold;color:red;">No Payment Made</p>';
			}
		}


		if($show_round_up){
			$round_up_total = ceil($real_total*20 - 0.5)/20;
		}
		...
		if ($result["warehouse_code"] != "--" && $result["warehouse_code"] != "") {
			$warehouse_icon = '&ensp; <i class="fas fa-warehouse"></i> '.$result["warehouse_code"];
		}else{
			$warehouse_icon = '&ensp; <i class="fas fa-warehouse"></i> -';
		...
		if($salesperson_uploads_exists){
			...
			if($db2->get_num_rows() > 0){
				...
				while($img = $db2->fetch_array())
				{
					...
					if(strpos($order_img, 'pdf') !== false){
						$img_div .= '<span class="attachment"  title="' . str_replace('"', '\"', $img["upload_remark"]) . '" style="height:100%;"><iframe src="https://docs.google.com/viewer?url='.$order_img.'&embedded=true"/></span>';
		...
		$edit_wh_button = '';
		if($allow_edit_warehouse){
			$edit_wh_button = '<button class="img_button" style="font-width: 10vw;margin-left:5px;" onclick="openWHModal(\''.$result['order_id'].'\')"><i class="fa fa-warehouse"></i></button>';
		}

		$edit_order_date_button = '';
		if($allow_edit_order_date){
			$display_order_date = date_create($result["order_date"]);
			$display_order_date = date_format($display_order_date,"Y-m-d");

			$edit_order_date_button = '<button class="img_button" style="font-width: 10vw;margin-left:5px;" onclick="openODModal(\''.$result['order_id'].'\',\''.$display_order_date.'\')"><i class="fa fa-calendar"></i></button>';
		}
		...
		$round_up_view = $show_round_up ? '<p class="title" style="color:black;font-size:13px;line-height:0;padding-top:5px"><strong style="font-size:14px;float:right;">'. $currency.number_format($round_up_total,2).' </strong></p>' : '';
		...
		if($result['order_status']=='0'){
			...
			<div style="cursor: pointer;">
				<div onclick="window.location=\'s_order_details.php?orderId='.$result["order_id"].'&client='.$client.'&isWarehouse='.$data['isWarehouse'].'\'">
			...
			$a_view .= $show_po_view ? $show_po_view : '';
			...
			if(in_array($data['client'],$delivery_module)){
				...
				if(!empty($select_order_approval) && !$isCustomer && $result['order_approved'] == 0 && $result["order_from"] == 'C'){
					...
					if($approve_comment_setting){
						$a_view .='<button data-client="'.$client.'" data-userid="'.$data['salesperson_id'].'"  data-orderid="'.$result['order_id'].'" class="radius non_important_text approve-button-st approve-button-comment display" style={width:25%;font-width: 10vw;}> Approve </button>';
					}else{
						$a_view .='<button data-client="'.$client.'" data-userid="'.$data['salesperson_id'].'"  data-orderid="'.$result['order_id'].'" class="radius non_important_text approve-button-st approve-button display" style={width:25%;font-width: 10vw;}> Approve </button>';
					}

					$a_view .= '<input style="float:right;" type="checkbox" class="checkedInvoice" id="checkedInvoice" name="checkedInvoice" value="checkedInvoice" data-client="'.$client.'" data-userid="'.$data['salesperson_id'].'"  data-orderid="'.$result['order_id'].'">';
				...
				$a_view .= $edit_wh_button;
				$a_view .= $edit_order_date_button;
				$a_view .= $payment_view;
			...
			$c_view .= $show_po_view ? $show_po_view : '';
			...
			if($isCustomer){
				if(isset($result['order_approved']) || $result['order_approved'] == 0){
					...
					if($result['order_approved'] == 1){
						...
						if($result["order_comment"]){
							$c_view .= '<br><p class="message non_important_text" style="color:#000">Comment: '.$result["order_comment"].' </p>';
						}
					}
					if($result['order_approved'] == 2){
						$c_view .= '
				...
				if($result['order_approved'] == 2){
					if($result["order_comment"]){
						$c_view .= '<p class="message non_important_text" style="color:rgb(255,85,45)">Reason: '.$result["order_comment"].' </p>';
					}
				}
			}
			$assigned = '';
			...
			if(in_array($data['client'],$delivery_module)){
				...
				if(!empty($select_order_approval) && !$isCustomer && $result['order_approved'] == 0 && $result["order_from"] == 'C'){
					...
					if(!empty($select_order_approval) && !$isCustomer && $result['order_approved'] == 0 && $result["order_from"] == 'S' && $approve_from_sp_setting){
						if($approve_comment_setting){
							$c_view .='<button data-client="'.$client.'" data-userid="'.$data['salesperson_id'].'"  data-orderid="'.$result['order_id'].'" class="radius non_important_text approve-button-st approve-button-comment" style={width:25%;font-width: 10vw;}> Approve </button>';
						}else{
							$c_view .='<button data-client="'.$client.'" data-userid="'.$data['salesperson_id'].'"  data-orderid="'.$result['order_id'].'" class="radius non_important_text approve-button-st approve-button" style={width:25%;font-width: 10vw;}> Approve </button>';
				...
				$c_view .= $edit_wh_button;
				$c_view .= $edit_order_date_button;
				$c_view .= $payment_view;
			...
			$t_view .= '
				<div style="cursor: pointer;">
					<div onclick="window.location=\'s_order_details.php?orderId='.$result["order_id"].'&client='.$client.'&isWarehouse='.$data['isWarehouse'].'\'"><p class="title" style="color:grey;font-size:13px;line-height:0;margin-top:5px"> '.$result["order_id"].$orderFrom.$greenRound;
			$t_view .=  !$hide_price || !$data['isWarehouse'] ? ' <strong style="font-size:14px;float:right;"> '.(in_array('grand_total',$order_list_module) ? '' : $currency.number_format($real_total,2).'').' </strong>' : '';
			$t_view .= $round_up_view;
			$t_view .= '</p><p class="title" > '.$display_company_name.(!empty($branch_name) ? " ({$branch_name})" : "").' </p>
						<p class="dates" ><i class="fas fa-clipboard-list"></i> '.$result["order_date"].' &ensp;<i class="fas fa-shipping-fast"></i> '.$result["delivery_date"].$warehouse_icon.'</p>';
			...
			$t_view .= $show_po_view ? $show_po_view : '';
			...
			$p_view .= $show_po_view ? $show_po_view : '';
	...
	$a_view = str_replace('@a_orders_count',$a_orders_count, $a_view);
	$c_view = str_replace('@c_orders_count',$c_orders_count, $c_view);
	$t_view = str_replace('@t_orders_count',$t_orders_count, $t_view);
	$p_view = str_replace('@p_orders_count',$p_orders_count, $p_view);
	...
	if(!$data['showCancel'] && ($checkConfirmed>0 || $checkTransferred>0 || $checkActiveCart>0 || $checkPosted>0) && !$data['isCustomerView'] && !$allow_constructor && (!$hide_price || !$data['isWarehouse'])){
