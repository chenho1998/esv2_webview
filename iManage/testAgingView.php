...
$ivKnockOff = "SELECT doc_ko_ref, invoice_amount AS 'CR', outstanding_amount,DATE(invoice_date) AS 'Date','Credit' AS 'Type', invoice_code AS 'Ref', IF(CONCAT(IFNULL(receipt_desc,''),IFNULL(cheque_no,'')) = '','INVOICE' AS description, '' AS 'DR',(outstanding_amount) AS 'OUTSTANDING', ko.doc_amount FROM cms_customer_ageing_ko AS ko LEFT JOIN cms_invoice iv ON iv.invoice_code = ko.doc_code WHERE doc_type = 'OR' AND cancelled = 'F' AND cust_code = '{$custCode}' AND doc_date >= '{$dateFrom}'";



echo $ivKnockOff.'<br><br>';
...
$dbClone->query($ivKnockOff);

if($dbClone->get_num_rows() != 0){
  while($ivRes = $dbClone->fetch_array()){
    $ko_ref = $ivRes['doc_ko_ref'];
    $doc_code = $ivRes['Ref'];
    $allKo[$ko_ref][] = $ivRes;
...
for ($i=0; $i < count($out_array); $i++) { 
  ...
  if(strtotime($row['Date']) <= strtotime($dateTo)){
    ...
    <td style="font-size:12px;">
        <p style="font-size:10px;margin:0px;font-weight:normal;text-align:center;">'.$row['Ref'].'</p>';
    if($row['cheque']){
        $table .=  '<p style="font-size:10px;margin:0px;font-weight:normal;text-align:center;">'.$row['cheque'].'</p>';
    }
...
for($k = 0; $k < 6; $k ++){
    ...
    echo '<br>'.$loop_inv_query.'<br>'.$loop_dn_query.'<br>'.$loop_cn_query.'<br>'.$loop_rp_query.'<br>';
