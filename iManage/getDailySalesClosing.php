...
//     <tbody> 
//       <tr>
//         <td class="order_date" colspan="7">Date: @date</td>
//       </tr>
//       @details
//     </tbody>
//   </table>
...
if(
    ...
    $agent_info = $mysql->Execute("SELECT * FROM cms_login");
    $agent_array = array();
    ...
    $diff=date_diff($dateFrom,$dateTo)->format("%a");
      ...
      $order_item_query = "SELECT product_code, product_name, ROUND(SUM(quantity),2) AS total, salesperson_id, driver_name FROM cms_order co LEFT JOIN cms_order_item coi ON coi.order_id = co.order_id WHERE delivery_date BETWEEN '".$date_from." 00:00:00' AND '".$date_to." 23:59:59' AND co.cancel_status = 0 AND coi.cancel_status = 0"; 
      ...
      $q = "SELECT ROUND(SUM(grand_total),2) AS total , driver_name, salesperson_id FROM cms_order WHERE delivery_date BETWEEN '".$date_from." 00:00:00' AND '".$date_to." 23:59:59' AND cancel_status = 0 AND doc_type ='cash' AND grand_total > 0 GROUP BY salesperson_id, driver_name";
      ...
      $q = "SELECT ROUND(SUM(grand_total),2) AS total , driver_name, salesperson_id FROM cms_order WHERE delivery_date BETWEEN '".$date." 00:00:00' AND '".$date." 23:59:59' AND cancel_status = 0 AND doc_type !='cash' AND grand_total > 0 GROUP BY salesperson_id, driver_name";
    ...
    // for ($k=0; $k <= $diff; $k++) { 
    //   $date = date("Y-m-d" , strtotime("+".$k." day", strtotime($date_from)));
    ...
    //   $order_item_query = "SELECT product_code, product_name, ROUND(SUM(quantity),2) AS total, salesperson_id, driver_name FROM cms_order co LEFT JOIN cms_order_item coi ON coi.order_id = co.order_id WHERE delivery_date BETWEEN '".$date." 00:00:00' AND '".$date." 23:59:59' AND co.cancel_status = 0 AND coi.cancel_status = 0"; 
