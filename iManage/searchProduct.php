...
function searchProduct($data){
	...
	$hide_aval_qty = $config['Hide_Aval_Qty'];
	$hide_aval_qty_setting = in_array($client,$hide_aval_qty['hide_aval_qty']);
	...
	if($db->get_num_rows() != 0)
	{
		...
		while($result = $db->fetch_array())
		{
			...
			if ($hide_aval_qty_setting){
				$label = "A";
				if ($result['product_current_quantity'] > 0.9){
					//$label = $result['product_current_quantity'];
				}
				if ($result['product_current_quantity'] <= 0){
					$label = "N";
				}
				$view .= '<p style="color:#147efb;font-weight:bold;">Aval Qty '. $label .'</p>';
			}
			if(!$hide_aval_qty_setting){
...
function addProduct($data){
	...
	if($db->get_num_rows() != 0)
	{
		while($result = $db->fetch_array())
		{
			$sql = "INSERT into cms_order_item (order_id,ipad_item_id,product_code,product_id,product_name,unit_uom,uom_id,sequence_no,packing_status,cancel_status,quantity,disc_1,disc_2,disc_3,discount_method,unit_price,isParent,parent_code,sub_total) SELECT '" . $data['orderId'] . "','" . $number . "','" . mysql_real_escape_string($result['product_code']) . "','" . $result['product_id'] . "','" . mysql_real_escape_string($result['product_name']) . "','" . mysql_real_escape_string($result['product_uom']) . "','0', MAX(sequence_no) + 1 AS sequence_no, '0','0','1','0','0','0','PercentDiscountType','" . $result['product_std_price'] . "','0','', '".$result['product_std_price']."' FROM cms_order_item WHERE order_id = '" . $data['orderId'] . "'";
		}
	}
	file_put_contents('searchProduct.log',$sql);
	if($db->query($sql)){

		$db->query("UPDATE cms_order SET grand_total = (SELECT SUM(sub_total) FROM cms_order_item WHERE order_id = '".$data['orderId']."' AND cancel_status = 0) WHERE order_id = '".$data['orderId']."'");
